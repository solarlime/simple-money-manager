...
router.post('/register', upload.none(), (request, response) => {
  ...
  if (password === '') { error.password = ['Поле Пароль обязательно для заполнения.']; }

  // если ошибка сформирована...
  if (JSON.stringify(error) !== '{}') { response.json({ success: false, error }); }// отправляем ошибку
...
router.get('/current', (request, response) => {
  const db = low(new FileSync('db.json'));// получение БД
  const { id } = request.query; // получение id пользователя из запроса
